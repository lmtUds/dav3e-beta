...
function info = pcaFeature()
    info.type = DataProcessingBlockTypes.FeatureExtraction;
    info.caption = 'Principal Component Analysis';
...
function [data,params] = apply(data,params)
    ...
    for i = 1:size(pos,1)
        ...
        cap(i) = sprintf('PCA_%.2f-%.2f',x(pos(i,1)),x(pos(i,2)));
    end
    ...
    if ~params.autoCoeffs
        for i=1:params.numCoeffs
           valueLabels(end+1) = string(['pcaCoeff#' num2str(i)]);
        end
    else
...
function feature = computeFeature(data,coeffs)
    if nargin<2
        [~, feature] = FeatureExtraction.extractHelpers.PCAReconstruct( data);
    else
    [~, feature] = FeatureExtraction.extractHelpers.PCAReconstruct( data, coeffs);
    end
end