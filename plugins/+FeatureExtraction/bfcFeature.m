...
function info = bfcFeature()
    info.type = DataProcessingBlockTypes.FeatureExtraction;
    info.caption = 'Best Fourier Coefficients';
...
function [data,params] = apply(data,params)
    ...
    for i = 1:size(pos,1)
        ...
        cap(i) = sprintf('BFC_%.2f-%.2f',x(pos(i,1)),x(pos(i,2)));
    end
    ...
    if ~params.autoCoeffs
        for i=1:params.numCoeffs
           valueLabels(end+1) = string(['bfcCoeff#' num2str(i)]);
        end
    else
...
function feature = computeFeature(data,coeffs)
    if nargin<2
        [~, feature] = FeatureExtraction.extractHelpers.BFCReconstruct( data);
    else
    [~, feature] = FeatureExtraction.extractHelpers.BFCReconstruct( data, coeffs);
    end
end