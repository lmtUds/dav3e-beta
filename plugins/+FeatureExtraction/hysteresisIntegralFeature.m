...
function info = hysteresisIntegralFeature()
    info.type = DataProcessingBlockTypes.FeatureExtraction;
    info.caption = 'hysteresis integral';
...
function [data,params] = apply(data,params)
    ...
    if mod(size(pos,1),2) == 0
        f = nan(size(d,1),size(pos,1)/2);
        cap = cell(1,size(pos,1)/2);
        for i = 1:2:size(pos,1)
            f(:,i) = computeFeature(...
                {d(:,pos(i,1):pos(i,2)),d(:,pos(i+1,1):pos(i+1,2))},...
                {x(pos(i,1):pos(i,2));x(pos(i+1,1):pos(i+1,2))});
            cap{i} = sprintf('hystInt_%d-%d',x(pos(i,1)),x(pos(i,2)));
        end
        params.header = cap;
    else
        f = nan(size(d,1),1);
