...
classdef Range < FeatureExtraction.extractHelpers.SignalChainElementInterface
    %RANGE Summary of this class goes here
    %   Detailed explanation goes here
    
    properties (Access = protected)
        selectedPoints = [];
        uid = '';
        %Boolean array used to index incoming signals
    end
    ...
    methods
        function this = Range(selectedPoints, uid, varargin)
            if nargin > 0
        ...
        function step(this, data, metaInfo)
            ...
            if ~isempty(this.next)
                this.next.step(data(:,this.selectedPoints), metaInfo);
            end
        end
        ...
        function info = getFeatInfo(this, info)
            if isempty(info)
                ...
                info.removeFeature(~this.selectedPoints)
            end
            info.addProperty('SignalChainId', this.getId());
            s = find(this.selectedPoints);
            info.addProperty('interval', [s(1), s(end)])
