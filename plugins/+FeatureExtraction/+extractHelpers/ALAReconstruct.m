...
function [ residuals, trainFeat, testFeat ] = ALAReconstruct( trainData, numSplits, testData )
...
errTrans = FeatureExtraction.extractHelpers.ALAErrTransform();
% numSplits = floor((numFeat-2)/2);
if nargin <2
    spl = FeatureExtraction.extractHelpers.ALASplit();
else
    spl = FeatureExtraction.extractHelpers.ALASplit(numSplits); %changed for DAVE; blank-> autoFind
end
ds = FeatureExtraction.extractHelpers.ALADownsampler();
featMat = FeatureExtraction.extractHelpers.RAMFeatureMat();

spl.addNextElement(featMat);
...
%getResiduals
rec = FeatureExtraction.extractHelpers.ALAReconstructor(spl.start, spl.stop, ds.dsFactor, sensor);
